...
def display_range_totals(client, dates_str_list):
    ...
    # TODO:
    # turn the two date strings in dates_str_list to datetime objects
    # and store in range_start_dt and range_end_dt

    format_string = "%Y-%m-%d"
    range_start_dt = datetime.datetime.strptime(dates_str_list[0], format_string)
    range_end_dt = datetime.datetime.strptime(dates_str_list[1], format_string).replace(
        hour=23, minute=59, second=59
    )

    # TODO: filter client_jobs to only include those 
    # within the start and end datetimes
    ...
    # TODO: List out all the different jobs, 
    # and then a total time spent - just like display_all_totals
